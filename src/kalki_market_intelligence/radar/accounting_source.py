"""Bounded accounting/compliance discovery from the SEC daily master index."""
...
MAXIMUM_ACCOUNTING_INDEX_ROWS = 5_000
...
def parse_accounting_master_index(
    document: SecRadarDocument,
) -> tuple[AccountingDiscoveryCandidate, ...]:
    """Group supported form rows by accession without guessing an issuer."""
    ...
    for line in document.body.decode("latin-1").splitlines():
        ...
        try:
            form = normalize_accounting_form(raw_form)
        ...
        if not name:
            continue
        selected_rows += 1
        if selected_rows > MAXIMUM_ACCOUNTING_INDEX_ROWS:
            raise AccountingParseError("accounting daily-index rows exceed the safety boundary")
