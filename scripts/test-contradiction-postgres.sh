...
container=kalki-contradiction-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable contradiction database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
for migration in \
    ...
    "$repository_dir"/migrations/002[0-5]_*.sql
do
...
if [ "$latest_version" != "0025_filing_change_lifecycle" ]; then
    echo "Disposable contradiction upgrade fixture is not at migration 0025." >&2
    exit 1
fi
docker exec --interactive "$container" psql --no-psqlrc --set=ON_ERROR_STOP=1 \
    --username postgres --dbname kalki_test \
    <"$repository_dir/migrations/0026_contradiction_receipts.sql" >/dev/null

published_port=$(docker port "$container" 5432/tcp | sed 's/^.*://')
KALKI_CONTRADICTION_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_CONTRADICTION_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_contradiction_postgres_integration.py"

echo "Disposable PostgreSQL contradiction receipt gate passed."
