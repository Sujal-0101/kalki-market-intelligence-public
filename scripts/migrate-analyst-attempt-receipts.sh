...
migration=$repository_dir/migrations/0010_analyst_attempt_receipts.sql
...
applied=$(docker compose -f compose.production.yaml exec -T postgres \
    ...
    --command "SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version = '0010_analyst_attempt_receipts')")
if [ "$applied" = "t" ]; then
    echo "Migration 0010 is already applied."
    exit 0
fi
...
echo "Migration 0010 applied. It has no destructive automatic rollback."
