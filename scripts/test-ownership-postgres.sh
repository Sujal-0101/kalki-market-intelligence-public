...
container=kalki-ownership-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable ownership database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_OWNERSHIP_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_OWNERSHIP_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_ownership_postgres_integration.py"

echo "Disposable PostgreSQL ownership gate passed."
