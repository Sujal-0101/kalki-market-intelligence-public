...
migration="$repository_dir/migrations/0023_sec_complete_submission_paths.sql"
container=${KALKI_POSTGRES_CONTAINER:-kalki-production-postgres-1}
remote_migration=/tmp/0023_sec_complete_submission_paths.sql
cleanup() {
...
if ! docker inspect --format '{{.State.Health.Status}}' "$container" | grep -qx healthy; then
    echo "Production PostgreSQL is not healthy; refusing migration 0023." >&2
    exit 1
fi
if docker exec "$container" psql --username kalki_owner --dbname kalki \
    --no-align --tuples-only --command \
    "SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version = '0023_sec_complete_submission_paths')" \
    | grep -qx t; then
    echo "Migration 0023 is already applied."
    exit 0
fi
...
echo "Migration 0023 applied. No candidate, link receipt, publication, or history was rewritten."
