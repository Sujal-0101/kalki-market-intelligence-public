...
container=kalki-filing-change-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable filing-change lifecycle database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
for migration in \
    "$repository_dir"/migrations/000[1-9]_*.sql \
    "$repository_dir"/migrations/001[0-9]_*.sql \
    "$repository_dir"/migrations/002[0-4]_*.sql
do
...
latest_version=$(docker exec "$container" psql --no-psqlrc --username postgres \
    --dbname kalki_test --tuples-only --no-align \
    --command "SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1")
if [ "$latest_version" != "0024_stale_analyst_attempt_recovery" ]; then
    echo "Disposable filing-change upgrade fixture is not at migration 0024." >&2
    exit 1
fi
docker exec --interactive "$container" psql --no-psqlrc --set=ON_ERROR_STOP=1 \
    --username postgres --dbname kalki_test \
    <"$repository_dir/migrations/0025_filing_change_lifecycle.sql" >/dev/null

published_port=$(docker port "$container" 5432/tcp | sed 's/^.*://')
KALKI_FILING_CHANGE_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_FILING_CHANGE_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_filing_change_lifecycle_postgres_integration.py"

echo "Disposable PostgreSQL filing-change lifecycle gate passed."
