...
container=kalki-accounting-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable accounting database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_ACCOUNTING_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_ACCOUNTING_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_accounting_postgres_integration.py"

echo "Disposable PostgreSQL accounting gate passed."
