...
container=kalki-novelty-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable event novelty database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_NOVELTY_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_NOVELTY_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_event_novelty_postgres_integration.py"

echo "Disposable PostgreSQL event novelty gate passed."
