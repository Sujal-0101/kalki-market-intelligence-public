...
container=kalki-focus-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable Focus database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_FOCUS_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_FOCUS_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_focus_postgres_integration.py"

echo "Disposable PostgreSQL Focus Universe gate passed."
