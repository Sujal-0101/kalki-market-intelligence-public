...
migration="$repository_dir/migrations/0027_outcome_science_invariants.sql"
container=${KALKI_POSTGRES_CONTAINER:-kalki-production-postgres-1}
remote_migration=/tmp/0027_outcome_science_invariants.sql
cleanup() {
...
if ! docker inspect --format '{{.State.Health.Status}}' "$container" | grep -qx healthy; then
    echo "Production PostgreSQL is not healthy; refusing migration 0027." >&2
    exit 1
fi
if docker exec "$container" psql --username kalki_owner --dbname kalki \
    --no-align --tuples-only --command \
    "SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version = '0027_outcome_science_invariants')" \
    | grep -qx t; then
    echo "Migration 0027 is already applied."
    exit 0
fi
...
if [ "$prior_versions" != "$expected_versions" ]; then
    echo "Production migration history is not the exact supported pre-0027 history." >&2
    exit 1
fi
ledger_counts=$(docker exec "$container" psql --username kalki_owner --dbname kalki \
    --no-align --tuples-only --field-separator '|' --command \
    "SELECT (SELECT count(*) FROM research_prospective_outcome_plans),
            (SELECT count(*) FROM research_prospective_outcome_jobs),
            (SELECT count(*) FROM research_prospective_outcome_attempts),
            (SELECT count(*) FROM research_prospective_outcomes)")
if [ "$ledger_counts" != '0|0|0|0' ]; then
    echo "Supporting-outcome history exists; refusing to install stricter invariants without review." >&2
    exit 1
fi
...
echo "Migration 0027 applied. No plan, attempt, outcome, publication, or history was rewritten."
