...
outcome_science_migration="$repository_dir/migrations/0027_outcome_science_invariants.sql"
image=postgres:18.0-alpine3.22@sha256:48c8ad3a7284b82be4482a52076d47d879fd6fb084a1cbfccbd551f9331b0e40
container=kalki-prospective-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable prospective-outcome database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
test -f "$outcome_science_migration"
for migration in "$repository_dir"/migrations/00[012][0-9]_*.sql
do
...
KALKI_PROSPECTIVE_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_PROSPECTIVE_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_prospective_outcomes.py" \
    "$repository_dir/tests/test_outcome_science.py" \
    "$repository_dir/tests/test_outcome_science_migration.py"

echo "Disposable PostgreSQL prospective-outcome gate passed."
