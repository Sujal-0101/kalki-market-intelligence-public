...
container=kalki-attempt-recovery-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable analyst-attempt recovery database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_ATTEMPT_GATE_PASSWORD_FILE="$temporary_dir/password" \
KALKI_ATTEMPT_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_analyst_attempt_recovery_postgres_integration.py"

echo "Disposable PostgreSQL analyst-attempt recovery gate passed."
