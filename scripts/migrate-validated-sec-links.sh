...
migration="$repository_dir/migrations/0020_validated_sec_links.sql"
container=${KALKI_POSTGRES_CONTAINER:-kalki-production-postgres-1}
remote_migration=/tmp/0020_validated_sec_links.sql
cleanup() {
...
if ! docker inspect --format '{{.State.Health.Status}}' "$container" | grep -qx healthy; then
    echo "Production PostgreSQL is not healthy; refusing migration 0020." >&2
    exit 1
fi
if docker exec "$container" psql --username kalki_owner --dbname kalki \
    --no-align --tuples-only --command \
    "SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE version = '0020_validated_sec_links')" \
    | grep -qx t; then
    echo "Migration 0020 is already applied."
    exit 0
fi
...
if [ "$prior_versions" != "$expected_versions" ]; then
    echo "Production migration history is not the exact supported pre-0020 history." >&2
    exit 1
fi
...
echo "Migration 0020 applied. Historical briefs were not rewritten or backfilled."
