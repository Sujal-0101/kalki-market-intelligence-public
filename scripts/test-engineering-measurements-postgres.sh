...
container=kalki-measurement-postgres-gate-$$
...
do
    ...
    if [ "$attempt" -ge 45 ]; then
        echo "Disposable engineering-measurement database did not become ready." >&2
        exit 1
    fi
    sleep 1
done
...
KALKI_MEASUREMENT_GATE_ENABLED=1 KALKI_MEASUREMENT_GATE_PORT="$published_port" \
    "$repository_dir/.venv/bin/pytest" -q \
    "$repository_dir/tests/test_engineering_measurements_postgres_integration.py"

echo "Disposable PostgreSQL engineering-measurement gate passed."
